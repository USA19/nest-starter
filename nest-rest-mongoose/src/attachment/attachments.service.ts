...
export class AttachmentsService {
  ...
  async uploadAttachment(file: File, updateAttachmentMediaInput: UpdateAttachmentMediaInput) {
    ...
    const attachments = await this.uploadMedia(file, updateAttachmentMediaInput, attachment._id)

    await this.updateAttachmentMedia(attachments, attachment._id)
    if (attachments.url) {
      return attachments
    }
  ...
  async updateAttachment(file: File, updateAttachmentMediaInput: UpdateAttachmentMediaInput) {
    ...
    if (existingAttachment) {
      ...
      if (deletedRemoteFile) {
        const attachments = await this.uploadMedia(file, updateAttachmentMediaInput, _id);

        if (attachments.url) {
          return await this.updateAttachmentMedia(attachments, _id)
        }
  ...
  async updateAttachmentMedia(updateAttachmentInput: UpdateAttachmentInput, id: string): Promise<Attachment> {
    try {
      return await this.attachmentsModel.findByIdAndUpdate(id, updateAttachmentInput, { new: true })
    } catch (error) {
