...
import { TemplateSwitch } from '../mailer/dto/dynamicTemplateData.dto';
 ...
 */
  ...
  async create(registerUserInput: RegisterUserInput): Promise<User> {
    try {
      ...
      await this.sendVerificationEmail(user, "verifyEmail");
      return user;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
   ...
   * @returns one user
   */
  async findOne(email: string, all = false): Promise<User | null> {
    const condition = { email };
    if (!all) condition["status"] = UserStatus.ACTIVE;
   ...
   * @returns by id
   */
  async findById(id: string, status?: UserStatus): Promise<User | null> {
  ...
  async sendVerificationEmail(user: User, templateName: TemplateSwitch = 'verifyEmail') {
    ...
    await this.emailService.sendEmail(email, fullName, token, templateName);
  }
   ...
   */
  async findRolesByUserId(id: string): Promise<User | null> {
  ...
  async forgotPassword(email: string): Promise<User | null> {
    try {
      ...
      if (user) {
        ...
        await this.emailService.sendEmail(email, fullName, token, "forgetPassword");
        return user;
      }
  ...
  async resendVerificationEmail(
    ...
    try {
      ...
      const user = await this.findOne(email.trim().toLowerCase(), true);
