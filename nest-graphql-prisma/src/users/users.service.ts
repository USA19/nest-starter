...
export class UsersService {
   ...
   * @returns update user
   */
  async update(updateUserInput: UpdateUserInput): Promise<UserPayload> {
    try {
   ...
   * @returns UserPayload
   */
  async updateRole(updateRoleInput: UpdateRoleInput): Promise<UserPayload> {
    try {
  ...
  async findAll(usersInput: UsersInput): Promise<UsersPayload> {
    try {
      const { from, to, roles, status, searchQuery, paginationOptions } = usersInput;
  ...
  async setNewPassword(
    ...
    const { newPassword, id } = updatePasswordInput;
